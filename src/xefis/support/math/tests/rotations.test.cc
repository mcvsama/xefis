...
AutoTest t1 ("Math: rotations 1", []{
	auto const vx = SpaceLength { 1_m, 0_m, 0_m };
	auto const vy = SpaceLength { 0_m, 1_m, 0_m };
	auto const vz = SpaceLength { 0_m, 0_m, 1_m };

	test_asserts::verify_equal_with_epsilon ("x → y", z_rotation_quaternion (+90_deg) * vx, vy, 1e-9_m);
	test_asserts::verify_equal_with_epsilon ("y → x", z_rotation_quaternion (-90_deg) * vy, vx, 1e-9_m);
	test_asserts::verify_equal_with_epsilon ("x → y → x", z_rotation_quaternion (+90_deg) * z_rotation_quaternion (-90_deg) * vx, vx, 1e-9_m);
	test_asserts::verify_equal_with_epsilon ("x → z", y_rotation_quaternion (-90_deg) * vx, vz, 1e-9_m);
	test_asserts::verify_equal_with_epsilon ("z → x", y_rotation_quaternion (+90_deg) * vz, vx, 1e-9_m);
	test_asserts::verify_equal_with_epsilon ("Z angle == 90_deg", rotation_angle (z_rotation_quaternion (+90_deg)), 90_deg, 1e-9_deg);
	test_asserts::verify_equal_with_epsilon ("Z axis == Z", rotation_axis (z_rotation_quaternion (+90_deg)), vz / 1_m, 1e-9);
});


AutoTest t2 ("Math: rotations 2", []{
