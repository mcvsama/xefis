	...
	public nu::Noncopyable
{
  ...
  public:
	...
	/**
	 * Set new acceleration moments excluding gravitational acceleration moments.
	 */
	template<CoordinateSystemConcept Space>
		void
		set_acceleration_moments_except_gravity (AccelerationMoments<Space> const&);

	/**
	 * Return total external force moments generated by this object at the center-of-mass.
	 * Constraint forces are not included here; they are computed separately by ImpulseSolver
	 * and accumulated in BodyIteration::all_constraints_force_moments.
	 */
	template<CoordinateSystemConcept Space>
		[[nodiscard]]
	...
	Body::mass_moments() const
	{
		...
		else if constexpr (std::is_same_v<Space, WorldSpace>)
		{
			...
			if (!_world_space_mass_moments)
				_world_space_mass_moments = _placement.rotate_to_base (_mass_moments);
	...
	Body::velocity_moments() const
	{
		...
		else if constexpr (std::is_same_v<Space, BodyCOM>)
		{
			...
			if (!_body_space_velocity_moments)
				_body_space_velocity_moments = _placement.rotate_to_body (_velocity_moments);
...
template<CoordinateSystemConcept Space>
	...
	Body::set_velocity_moments (VelocityMoments<Space> const& velocity_moments)
	{
		...
		if constexpr (std::is_same_v<Space, WorldSpace>)
			...
			_velocity_moments = _placement.rotate_to_base (velocity_moments);
		else
			static_assert (false, "unsupported coordinate system");
	}
	...
	Body::acceleration_moments() const
	{
		...
		else if constexpr (std::is_same_v<Space, BodyCOM>)
		{
			...
			if (!_body_space_acceleration_moments)
				_body_space_acceleration_moments = _placement.rotate_to_body (_acceleration_moments);
...
template<CoordinateSystemConcept Space>
	...
	Body::set_acceleration_moments (AccelerationMoments<Space> const& acceleration_moments)
	{
		...
		if constexpr (std::is_same_v<Space, WorldSpace>)
			...
			_acceleration_moments = _placement.rotate_to_base (acceleration_moments);
		else
			static_assert (false, "unsupported coordinate system");
	}
	...
	Body::external_force_moments() const
	{
		if constexpr (std::is_same_v<Space, WorldSpace>)
		{
			...
			if (!_world_space_applied_impulses)
				_world_space_applied_impulses = _placement.rotate_to_base (_applied_impulses);
...
template<CoordinateSystemConcept Space>
	...
	Body::apply_impulse (ForceMoments<Space> const& force_moments)
	{
		...
		if constexpr (std::is_same_v<Space, WorldSpace>)
			_applied_impulses += _placement.rotate_to_body (force_moments);
		else if constexpr (std::is_same_v<Space, BodyCOM>)
...
template<CoordinateSystemConcept ForceSpace, CoordinateSystemConcept PositionSpace>
	...
	Body::apply_impulse (ForceMoments<ForceSpace> const& force_moments, SpaceLength<PositionSpace> const& position)
	{
		...
		if constexpr (std::is_same_v<ForceSpace, WorldSpace>)
			body_space_force_moments = _placement.rotate_to_body (force_moments);
...
template<CoordinateSystemConcept Space>
	...
	Body::apply_impulse (Wrench<Space> const& wrench)
	{
		...
		if constexpr (std::is_same_v<Space, WorldSpace>)
		{
			...
			_applied_impulses += resultant_force (_placement.rotate_translate_to_body (wrench));
		}
		else if constexpr (std::is_same_v<Space, BodyCOM>)
