...
template<class Unit>
	struct SocketTraits<si::Quantity<Unit>>
	{
		...
		to_string (Socket<si::Quantity<Unit>> const& socket, SocketConversionSettings const& settings)
		{
			if (socket)
			{
				if (settings.preferred_units.empty())
					return std::format (settings.numeric_format_double, socket->to_floating_point()) + unit_suffix (*socket);
				else
				{
					for (si::DynamicUnit const& du: settings.preferred_units)
						if (si::is_convertible (Unit::dynamic_unit(), du))
							return std::format (settings.numeric_format_double, si::convert (Unit::dynamic_unit(), socket->to_floating_point(), du)) + " " + du.symbol();
		...
		{
			if (socket)
				return static_cast<float128_t> (socket->to_floating_point());
			else
				return std::nullopt;
		}
