	...
	_screen (screen)
{
	...
	auto [name_strip, name_label] = create_colored_strip_label (nu::to_qstring (_screen.instance()).toHtmlEscaped(), QColor (0xff, 0xaa, 0x00), Qt::AlignBottom, this);
...
ScreenWidget::create_performance_tab()
{
...

	{
		...
		for (auto& widgets_pair: _work_performer_widgets)
		{
			...
			auto* handled_modules_info = new QLabel ("<b>Modules handled by this work performer:</b><br/>" + nu::to_qstring (widgets.module_names).toHtmlEscaped(), tab);
			...
			std::tie (widgets.start_latency_histogram, widgets.start_latency_stats, widgets.start_latency_group)
				= create_performance_widget (tab, nu::to_qstring ("Paint start latency (request start to painting start)"));

			std::tie (widgets.total_latency_histogram, widgets.total_latency_stats, widgets.total_latency_group)
				= create_performance_widget (tab, nu::to_qstring ("Total latency (request start to painting finish)"));
