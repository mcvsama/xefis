...
EngineTorque::data_updated()
{
	if (_input_engine_rpm.fresh())
	{
		if (_input_engine_rpm.valid())
		{
			...
			{
				AngularVelocity engine_spd = *_input_engine_rpm;
				AngularAcceleration derivative = (engine_spd - **_previous_engine_spd) / (update_time() - _previous_engine_spd->update_time());
				// Invalid computation, but needs only to be proportional:
				// TODO this should be done properly (include forces generated by ailerons at a specific airspeed).
				_output_engine_torque = 1_Nm * (_derivative_coefficient * derivative.quantity<RadianPerSecondSquared>() + _linear_coefficient * engine_spd.quantity<RadianPerSecond>());
			}
			else
